...
"""
#!!!  Не до конца понял, почему в примере они прописали @blueprint_numbers.route (выше), подозреваю, что это для описания методов в /docs, но я попытался как-то это сделать и не получилось Как описать метод в docs? Как добавить метод в какой-то созданный тег?
